...
export default function SingleCertificatePage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = use(params);
  const router = useRouter();
  const certificate = certificates.find((c) => c.slug === resolvedParams.slug);
        ...
        >
          ...
          >
            ...
            <motion.div 
              ...
              className="relative aspect-video border-[2px] md:border-[8px] border-stone-100 bg-stone-900 overflow-hidden shadow-[20px_20px_0px_rgba(255,255,255,0.02)] transform-style-3d perspective-1000 p-8 md:p-16 flex items-center justify-center"
            >
              <div className="relative w-full h-full max-w-[70%] max-h-[70%] mx-auto">
                <Image
                  src={certificate.image}
                  className="object-contain w-full h-full opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700"
          ...
          >
            <div className="space-y-2">
              ...
              >
                Issuer: {certificate.issuer} {"//"} Issued: {certificate.issueDate}
              </motion.p>
              {certificate.credentialId && (
                <motion.p 
                  ...
                  className="text-[10px] md:text-xs font-black uppercase tracking-[0.1em] text-stone-600"
                >
            ...
            >
              {certificate.skills.split(',').map((s) => s.trim()).map((s, i) => (
                <motion.span
                  key={s}
            ...
            >
              {certificate.credentialUrl ? (
                ...
                >
                  VERIFY CERTIFICATE ↗ <span className="group-hover/link:translate-x-2 transition-transform text-red-900">→</span>
                </a>
              ) : (
                <div className="inline-flex items-center gap-4 font-bebas text-2xl md:text-2xl text-stone-600">
                  NOT AVAILABLE
                </div>
              )}
            </motion.div>
